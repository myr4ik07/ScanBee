...
namespace ScanBee
{
    public class Exchange
    {
        ...
        public async Task<bool> GetDocumentsList()
        {
            ...
            try
            {
                FillDocumentsList(new JsonSerialization().deserializeJSON<IputStructure>((string)res.MinorStatus), (MainPage)Context);
            }
            catch
            {
        ...
        public async Task<bool> GetGoodList(string DocGuid)
        {
            ...
            try
            {
                FilGoodsList(new JsonSerialization().deserializeJSON<IputStructure>((string)res.MinorStatus), (Page_GoodsList)Context);
            }
            catch
            {
        ...
        public async Task<bool> DeleteItem(OutputStructure locOS)
        {
            ...
            try
            {
                IputStructure IS = new JsonSerialization().deserializeJSON<IputStructure>((string)res.MinorStatus);
                if (IS.Status != "ok")
                {
                    await ((ContentPage)Context).DisplayAlert("Error!", (string)res.MinorStatus.ToString(), "ok");
                    return false;
        ...
        public async Task<bool> GetItemListByFilter(ContentPage Context, ItemsRequestStructure P, string Filter)
        {
            ...
            try
            {
                FillObjectsList(new JsonSerialization().deserializeJSON<IputStructure>((string)res.MinorStatus), (ItemList)Context,P);
            }
            catch
            {
                await Context.DisplayAlert("Error!", "Ошибка при разборке полученного пакета!", "ok");
        ...
        public async Task<bool> GetGoodByBarcode(string Barcode)
        {
            ...
            try
            {
                ReplaceObjectInGoodRedactingPage(new JsonSerialization().deserializeJSON<IputStructure>((string)res.MinorStatus));
            }
            catch
            {
                await Context.DisplayAlert("Error!", "Ошибка при разборке полученного пакета!", "ok");
        ...
        public void FilGoodsList(IputStructure IS, Page_GoodsList Context)
        {
        ...
        public async Task<Status> HttpRequest(string Params, string HttpAddr, string Login, string Password, ContentPage Context)
        {
            ...
            using (HttpClient client = new HttpClient { Timeout = TimeSpan.FromSeconds(Convert.ToInt32(new CommonProcs().GetProperty("ext_Timeout"))) })
            {
                using (HttpRequestMessage request = new HttpRequestMessage())
                {
                    ...
                    request.Content = new StringContent(Params);
                    ...
                    try
                    {
                        ...
                        if (response.StatusCode == HttpStatusCode.OK)
                        {
                            ...
                            ret.MinorStatus = await responseContent.ReadAsStringAsync();
        ...
        public void FillDocumentsList(IputStructure IS, MainPage Context)
        {
        ...
        public void FillObjectsList(IputStructure IS, ItemList Page_IL, ItemsRequestStructure locParent)
        {
        ...
        public void ReplaceObjectInGoodRedactingPage(IputStructure IS)
        {
            foreach (TablePart TP in IS.Table0)
            {
    ...
    [DataContract]
    public class IputStructure
    {
        public IputStructure()
        {
            Table0 = new List<TablePart>();
            return;
        }
